...
def importClass(className, modType):
    '''
    Helper function for importing cut, result, and cleaner classes by name.
    className is the name of the class (which should also be the name of the 
    file it's in, modulo a '.py').
    modType should indicate whether it's for a cut class, a result class, or
    a cleaner class.
    '''
    if 'cut' in modType.lower():
        zzaDir = 'cuts/cutTemplates'
    elif 'result' in modType.lower():
        zzaDir = 'results/resultTemplates'
    elif 'clean' in modType.lower():
        zzaDir = 'cleaning/rowCleaners'
    else:
        raise ImportError('The import helper only knows to to import cut, '
                          'result, and cleaner modules. I don\'t know what '
                          'to do about {}'.format(modType))

    modName = os.path.join('ZZAnalyzer', zzaDir, className)
    ...
    try:
        ...
        print ("There doesn't seem to be a {0} module "
               "called {1}").format(modType, className)
        raise
    else:
        mod = imp.load_module(className, f, p, d)
