...
####
puDataSetID = "29Feb2016" # identifier for all PU data root files
histname = 'pileup'
baseFileName = os.path.join(os.environ["zza"], 'ZZAnalyzer', 'data/pileupReweighting', "PileUpData_%s{0}.root"%puDataSetID)
scaleSuffixes = {
    '' : '', # no scaling
    'ScaleUp' : '_up',
    'ScaleDown' : '_down',
}
outputFile = os.path.join(os.environ["zza"], 'ZZAnalyzer', 'data/pileupReweighting', "PUScaleFactors_{0}.root".format(puDataSetID))


# Get PU distribution used in simulation
puDistPath = os.path.join(os.environ["zza"],'ZZAnalyzer',"data/pileupReweighting")
