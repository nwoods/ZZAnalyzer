...
for ob in objects:
    ...
    t.Draw("%sPt>>h%sDenom(40, 0., 100.)"%(ob,ob), "%sPt>5.&&fabs(%sEta)<2.4&&(%sIsGlobal||%sIsTracker)&&abs(%sGenPdgId)==13%s&&%sComesFromHiggs==0"%(ob,ob,ob,ob,ob,zmassStr,ob), "goff")
    t.Draw("%sPt>>h%sNum(40, 0., 100.)"%(ob,ob), "%sPt>5.&&fabs(%sEta)<2.4&&(%sIsGlobal||%sIsTracker)&&abs(%sGenPdgId)==13%s&&%sComesFromHiggs==0&&%sRelPFIsoDBDefault<0.4"%(ob,ob,ob,ob,ob,zmassStr,ob,ob), "goff")
    #%sSIP3D<4.&&
...
saveFile = ROOT.TFile("/home/nwoods/UWCMS/ZZ/ZZAnalyzer/plots/SMPCombo_summary/isoEffPt_8TeV.root","RECREATE")

newbins = array.array('d', [5, 10, 15, 20, 30, 40, 100])
...
frame.GetXaxis().SetTitle("#mu pt")
frame.GetYaxis().SetTitle("Iso < 0.4 Efficiency")

frame.Draw()
eff.Draw("PE1")

c.Print("/home/nwoods/UWCMS/ZZ/ZZAnalyzer/plots/4muIsoEff_8TeV.png")
