...
Cutter for the 2015 H->ZZ->4l analysis with akT FSR with R=0.15.
...
class FullSpectrum_AKFSR1p5(FullSpectrum_FullFSR_Sync):
    def __init__(self, cutset="FullSpectrum_AKFSR1p5"):
        super(FullSpectrum_AKFSR1p5, self).__init__(cutset)
    ...
    def getCutTemplate(self, *args):
        ...
        Use AK FSR
        '''
        temp = super(FullSpectrum_AKFSR1p5, self).getCutTemplate()

        del temp['mIso']['cuts']['RelPFIsoDBDefaultFSR']
        temp['mIso']['cuts']['RelPFIsoDBAKFSR1p5'] = (0.4, "<")
        del temp['eIso']['cuts']['RelPFIsoRhoFSR']
        temp['eIso']['cuts']['RelPFIsoRhoAKFSR1p5'] = (0.5, "<")

        del temp['ZMassLoose']['cuts']['MassFSR#lower']
        del temp['ZMassLoose']['cuts']['MassFSR#upper']
        temp['ZMassLoose']['cuts']['MassAKFSR1p5#lower'] = (12., '>=')
        temp['ZMassLoose']['cuts']['MassAKFSR1p5#upper'] = (120., '<')

        del temp['ZMassTight']['cuts']['MassFSR#lower']
        temp['ZMassTight']['cuts']['MassAKFSR1p5#lower'] = (40, ">=")
        del temp['ZMassTight']['cuts']['MassFSR#upper']
        temp['ZMassTight']['cuts']['MassAKFSR1p5#upper'] = (120, "<")

        del temp['4lMass']['cuts']['MassFSR']

        temp['4lMass']['cuts']['MassAKFSR1p5'] = (70, ">=")
    ...
    def doSmartCut(self, row, *obj):
        ...
        altZMass = [nObjVar(row, "MassAKFSR1p5", *sorted(altObj[:2])), nObjVar(row, "MassAKFSR1p5", *sorted(altObj[2:]))]
        altZCompatibility = [zMassDist(m) for m in altZMass]
        z1Compatibility = zCompatibility(row, obj[0], obj[1], fsrType="AKFSR1p5")
